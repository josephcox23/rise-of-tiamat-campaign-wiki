...
A good example of a transformer plugin that borrows from the `remark` and `rehype` ecosystems is the [[x_docs/plugins/Latex|Latex]] plugin:
...
> [!tip]
> Both `resolve` and `prune` fall back to `quartz.config.default.yaml` if no `quartz.config.yaml` is present. This is useful for CI environments where the default config is the source of truth. See [[x_docs/cli/plugin#prune|prune]] and [[x_docs/cli/plugin#resolve|resolve]] for full details.
