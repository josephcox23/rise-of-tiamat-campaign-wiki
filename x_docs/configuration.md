...
Both commands support `--dry-run` to preview changes. See [[x_docs/cli/plugin|the plugin CLI reference]] for full details.
...
You can see a list of all plugins and their configuration options [[x_docs/tags/plugin|here]].
...
> [!tip]
> For per-heading font control, self-hosted fonts, or Obsidian theme font bridging, see the [[Fonts|Fonts]] plugin. It can download Google Fonts at build time and serve them locally with `fontOrigin: selfHosted`, making your site fully self-contained.
